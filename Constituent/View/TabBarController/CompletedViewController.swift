...
//        self.addCustomizedBackBtn(title:"  Meeting")
    ...
    func setMeeting(meeting: [MeetingData]) {
        ...
        for data in meetingeData {
            ...
            if statusArr.contains("COMPLETED") {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! CompletedCell
...
//           cell.meetingOnDate.text = String(meeting_DateArr[indexPath.row])
 ...
 func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
     ...
     self.performSegue(withIdentifier: "toMeetingDetail2", sender: self)
   }
}

extension CompletedViewController: MeetingView
{
    func startLoading() {
 ...
 override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
     ...
     if (segue.identifier == "toMeetingDetail2")
     {
